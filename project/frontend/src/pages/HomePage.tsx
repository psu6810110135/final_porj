...
function RichDropdown({
  ...
  const filtered = options.filter((o) => {
    ...
    const matchEnglish = o.searchTerms
      ? o.searchTerms.toLowerCase().includes(s)
      : false;
  ...
  return (
    <div className="flex-1 relative min-w-[0]">
      <button
        onClick={onToggle}
        className={`w-full bg-white rounded-xl sm:rounded-full px-3 md:px-4 py-2.5 md:py-3 flex items-center gap-2 transition-all duration-200 ${isOpen ? "ring-2 ring-[#FF8400] shadow-md bg-white" : "border border-[#4F200D]/15 hover:border-[#FF8400]/50 hover:shadow-sm"}`}
      >
        <span
      ...
      <div
        className={`absolute top-full left-0 right-0 mt-1.5 bg-white rounded-2xl shadow-2xl border border-[#F0E8E0] overflow-hidden z-50 transition-all duration-200 origin-top ${isOpen ? "opacity-100 scale-y-100 translate-y-0" : "opacity-0 scale-y-95 -translate-y-1 pointer-events-none"}`}
      >
        ...
        <div className="max-h-56 overflow-y-auto py-1 relative custom-scrollbar">
          {!search && (
            <button
              onClick={() => onSelect("")}
              className={`w-full px-4 py-2.5 text-left text-sm flex items-center gap-2 transition-colors ${!value ? "bg-[#FF8400]/8 text-[#FF8400] font-bold" : "text-[#4F200D]/60 hover:bg-[#F6F1E9]"}`}
            >
              {!value && (
          ...
          {filtered.length > 0 ? (
            filtered.map((opt) => (
              <button
                ...
                className={`w-full px-4 py-2.5 text-left text-sm flex items-center gap-2 transition-colors ${value === opt.value ? "bg-[#FF8400]/8 text-[#FF8400] font-bold" : "text-[#4F200D] hover:bg-[#F6F1E9]"}`}
              >
                {value === opt.value && (
                  <span className="w-1.5 h-1.5 rounded-full bg-[#FF8400]" />
                )}{" "}
            ...
            <div className="px-4 py-4 text-sm text-[#4F200D]/50 text-center font-medium">
              ไม่พบข้อมูล "{search}"
            </div>
          )}
        </div>
  ...
  const defaultDestinations: TourCardData[] = [
    {
      ...
      description: "ดำน้ำดูปะการังระดับโลก",
    ...
    },
    {
      id: 2,
      title: "เขาสก",
      description: "นอนแพพักผ่อน ท่ามกลางป่าฝน",
    ...
    },
    {
      id: 3,
      title: "ภูเก็ต",
      description: "สัมผัสความโรแมนติกในแดนสวรรค์",
    ...
    fetch(`${API_BASE}/tours/recommended`)
      ...
      .then((data: any[]) => {
        if (Array.isArray(data) && data.length > 0)
          setTopTours(processTours(data));
        else
          fetch(`${API_BASE}/tours`)
            .then((r) => r.json())
            .then((all) => {
              if (Array.isArray(all) && all.length > 0)
                setTopTours(processTours(all.slice(0, 3)));
            });
      })
        ...
        const mapped = data
          ...
          .map((review) => {
            ...
            return {
              ...
              avatarUrl: getAvatarUrl(u.avatar_url ?? u.avatarUrl ?? ""),
            };
          })
    ...
    <div className="min-h-screen bg-[#F6F1E9]">
      ...
      <section className="relative min-h-[320px] md:min-h-[400px] lg:min-h-[480px] overflow-visible">
        ...
        <div className="relative z-10 flex flex-col items-center justify-center min-h-[320px] md:min-h-[400px] lg:min-h-[480px] px-4 py-8">
          ...
          <div
            className={`search-bar-container w-full max-w-full sm:max-w-2xl md:max-w-4xl xl:max-w-5xl px-2 sm:px-0 transition-all duration-700 delay-500 ${heroReady ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
          >
            ...
            <div className="order-2 lg:order-1">
              ...
              <div className="space-y-5 md:space-y-6">
                {steps.map((step, idx) => (
                  <div
                    key={step.num}
                    className={`flex items-start gap-3 md:gap-5 transition-all duration-500 ${planReveal.visible ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-6"}`}
                  ...
                  >
                    <CardContent className="p-4 md:p-6">
                      <div className="flex items-center gap-3 mb-3 md:mb-4">
                        <div className="w-12 h-12 md:w-14 md:h-14 rounded-full bg-gradient-to-br from-[#FF8400] to-[#FFD93D] flex items-center justify-center flex-shrink-0 shadow-sm overflow-hidden">
                          {testimonial.avatarUrl &&
                          !brokenAvatarIds[testimonial.id] ? (
                            <img
