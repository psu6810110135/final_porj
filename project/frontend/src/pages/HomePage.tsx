...
export default function HomePage() {
  ...
  useEffect(() => {
    fetch("http://localhost:3000/api/v1/tours")
      .then((res) => res.json())
      .then((data) => {
        const uniqueProvinces = [...new Set(data.map((tour: any) => tour.province))].filter(Boolean) as string[];
        const uniqueCategories = [...new Set(data.map((tour: any) => tour.category))].filter(Boolean) as string[];
        const uniqueDurations = [...new Set(data.map((tour: any) => tour.duration))].filter(Boolean) as string[];
  ...
  const testimonials = [
    { id: 1, name: "นเรศ วงวิไล", content: "ประสบการณ์เที่ยวที่ดีที่สุด! ไกด์น่ารักมาก วิวสวยหลักล้าน ทุกอย่างจัดการได้เป๊ะมาก" },
    { id: 2, name: "ยศธร รัตนาประสิทธิ์", content: "จองง่ายมาก แอปช่วยวางแผนทริปได้แบบไม่ต้องกังวลเลย แนะนำแพ็กเกจเขาสกมาก ๆ" },
    { id: 3, name: "จันทรวิมล พงษ์ธนาพัฒน์", content: "แนะนำเลยสำหรับครอบครัว เด็ก ๆ ชอบกิจกรรมที่ภูเก็ตมาก ปีหน้ามาจองซ้ำแน่นอน!" },
  ];

  const destinations = [
    { id: 1, name: "หมู่เกาะสิมิลัน", description: "ดำน้ำดูปะการังระดับโลก น้ำใสหาดทรายขาว", image: "/src/assets/หมู่เกาะสิมิลัน.jpeg", duration: "ทริป 1 วัน", price: "฿3,500" },
    { id: 2, name: "เขาสก", description: "นอนแพพักผ่อน ท่ามกลางป่าฝนอันอุดมสมบูรณ์", image: "/src/assets/เขาสก.jpeg", duration: "3 วัน / 2 คืน", price: "฿8,900" },
    { id: 3, name: "ภูเก็ต", description: "สัมผัสความโรแมนติกในแดนสวรรค์เขตร้อน", image: "/src/assets/ภูเก็ต.jpeg", duration: "ทริป 1 วัน", price: "฿2,200" },
  ];
    ...
    <div className="min-h-screen bg-[#F6F1E9]">
      ...
      <section className="relative min-h-[300px] md:min-h-[375px] lg:min-h-[450px] overflow-hidden">
        <div className="absolute inset-0">
          <img src="/src/assets/bg2.jpeg" alt="Thailand Tourism" className="w-full h-full object-cover object-center"
            onError={(e) => { e.currentTarget.style.display = "none"; e.currentTarget.parentElement!.style.background = "linear-gradient(135deg, #4F200D 0%, #8B4513 100%)"; }} />
        ...
        <div className="relative z-10 flex flex-col items-center justify-center min-h-[300px] md:min-h-[375px] lg:min-h-[450px] px-4 py-6">
          ...
          <div className="w-full max-w-2xl md:max-w-4xl xl:max-w-6xl mt-6 md:mt-12">
            <div className="dropdown-container bg-[#F6F1E9]/95 backdrop-blur-sm rounded-full p-2 flex flex-col sm:flex-row gap-2 border-2 border-[#E3DCD4] items-center relative">
              {/* Province Dropdown */}
              <div className="flex-1 relative">
                <button onClick={() => { setShowProvinceDropdown(!showProvinceDropdown); setShowCategoryDropdown(false); setShowDurationDropdown(false); }}
                  ...
                  <span className="text-xs md:text-sm lg:text-base text-[#4F200D]/90 font-medium truncate">{selectedProvince || "เลือกจังหวัด"}</span>
                  <ChevronDownIcon />
                </button>
                {showProvinceDropdown && (
                  <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-2xl shadow-xl border border-[#E3DCD4] max-h-60 overflow-y-auto z-50">
                    <button onClick={() => { setSelectedProvince(""); setShowProvinceDropdown(false); }} className="w-full px-4 py-3 text-left text-sm text-[#4F200D]/70 hover:bg-[#F6F1E9] transition-colors">ทั้งหมด</button>
                    {provinces.map((province) => (
                      <button key={province} onClick={() => { setSelectedProvince(province); setShowProvinceDropdown(false); }} className="w-full px-4 py-3 text-left text-sm text-[#4F200D] hover:bg-[#F6F1E9] transition-colors">{province}</button>
                    ))}
                  </div>
                )}
              </div>
              ...
              {/* Category Dropdown */}
              <div className="flex-1 relative">
                <button onClick={() => { setShowCategoryDropdown(!showCategoryDropdown); setShowProvinceDropdown(false); setShowDurationDropdown(false); }}
                  className="w-full bg-[#FFFDFA] rounded-full px-2 md:px-3 py-2 md:py-3 flex items-center justify-between border-2 border-[#4F200D]/30 hover:border-[#4F200D] transition-colors">
                  <span className="text-xs md:text-sm lg:text-base text-[#4F200D]/90 font-medium truncate">{selectedCategory || "เลือกประเภททัวร์"}</span>
                  <ChevronDownIcon />
                </button>
                {showCategoryDropdown && (
                  <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-2xl shadow-xl border border-[#E3DCD4] max-h-60 overflow-y-auto z-50">
                    <button onClick={() => { setSelectedCategory(""); setShowCategoryDropdown(false); }} className="w-full px-4 py-3 text-left text-sm text-[#4F200D]/70 hover:bg-[#F6F1E9] transition-colors">ทั้งหมด</button>
                    {categories.map((category) => (
                      <button key={category} onClick={() => { setSelectedCategory(category); setShowCategoryDropdown(false); }} className="w-full px-4 py-3 text-left text-sm text-[#4F200D] hover:bg-[#F6F1E9] transition-colors">{category}</button>
                    ))}
                  </div>
                )}
              </div>
              ...
              {/* Duration Dropdown */}
              <div className="flex-1 relative">
                <button onClick={() => { setShowDurationDropdown(!showDurationDropdown); setShowProvinceDropdown(false); setShowCategoryDropdown(false); }}
                  className="w-full bg-[#FFFDFA] rounded-full px-2 md:px-3 py-2 md:py-3 flex items-center justify-between border-2 border-[#4F200D]/30 hover:border-[#4F200D] transition-colors">
                  <span className="text-xs md:text-sm lg:text-base text-[#4F200D]/90 font-medium truncate">{selectedDuration || "จำนวนวันเดินทาง"}</span>
                  <ChevronDownIcon />
                </button>
                {showDurationDropdown && (
                  <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-2xl shadow-xl border border-[#E3DCD4] max-h-60 overflow-y-auto z-50">
                    <button onClick={() => { setSelectedDuration(""); setShowDurationDropdown(false); }} className="w-full px-4 py-3 text-left text-sm text-[#4F200D]/70 hover:bg-[#F6F1E9] transition-colors">ทั้งหมด</button>
                    {durations.map((duration) => (
                      <button key={duration} onClick={() => { setSelectedDuration(duration); setShowDurationDropdown(false); }} className="w-full px-4 py-3 text-left text-sm text-[#4F200D] hover:bg-[#F6F1E9] transition-colors">{duration}</button>
                    ))}
                  </div>
                )}
              </div>

              <Button onClick={handleSearch} className="bg-[#FF8400] text-white hover:bg-[#FF8400]/90 rounded-full px-4 md:px-6 py-2 md:py-3 text-xs md:text-sm lg:text-base font-bold shadow-lg whitespace-nowrap">
                ค้นหา
              </Button>
            </div>
          </div>
            ...
            {features.map((feature) => (
              <Card key={feature.id} className="border-0 shadow-lg rounded-2xl md:rounded-3xl bg-[#FFFDFA] overflow-hidden">
                <CardContent className="p-5 md:p-6 text-center">
                  ...
                  <h3 className="text-base md:text-lg lg:text-2xl font-bold text-[#4F200D] mb-2">{feature.title}</h3>
      ...
      <section className="relative py-18 md:py-24 lg:py-36 overflow-hidden">
        <div className="absolute inset-0">
          <img src="/src/assets/bg1.jpeg" alt="Travel Background" className="w-full h-full object-cover"
            onError={(e) => { e.currentTarget.style.display = "none"; e.currentTarget.parentElement!.style.background = "linear-gradient(135deg, #FF8400 0%, #FFD93D 100%)"; }} />
          <div className="absolute inset-0 bg-black/40"></div>
        </div>
        <div className="absolute top-3 md:top-6 left-3 md:left-8 opacity-30"><div className="rotate-180"><QuoteIcon /></div></div>
        <div className="absolute bottom-3 md:bottom-6 right-3 md:right-8 opacity-30"><QuoteIcon /></div>
        <div className="relative z-10 max-w-[1920px] mx-auto px-4 md:px-8">
          <div className="text-center">
            <h2 className="text-lg md:text-3xl lg:text-5xl xl:text-[96px] font-bold text-white leading-tight mb-6 md:mb-12 drop-shadow-lg px-4">
              การเดินทาง คือการลงทุนเดียว<br />ที่ทำให้ชีวิตคุณมั่งคั่งขึ้น
            </h2>
          ...
          <div className="grid lg:grid-cols-2 gap-6 md:gap-9 items-start">
            <div className="order-2 lg:order-1">
              ...
              <div className="space-y-5 md:space-y-6">
                {steps.map((step) => (
                  <div key={step.num} className="flex items-start gap-3 md:gap-5">
                    <div className="w-12 h-12 md:w-18 md:h-18 flex-shrink-0 rounded-xl md:rounded-2xl bg-[#F6F1E9] flex items-center justify-center">
                      <span className="text-lg md:text-3xl font-bold text-[#FF8400]">{step.num}</span>
                    </div>
                    <div>
            ...
            <div className="order-1 lg:order-2">
              <div className="relative rounded-2xl md:rounded-3xl overflow-hidden shadow-2xl">
                <img src="https://images.unsplash.com/photo-1501785888041-af3ef285b470?w=800&q=80" alt="Trip Planning" className="w-full h-48 md:h-72 lg:h-[375px] xl:h-[450px] object-cover" />
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-9 md:py-12 lg:py-18 bg-[#FFFDFA]">
        <div className="max-w-[1920px] mx-auto px-4 md:px-8">
          <div className="text-center mb-9 md:mb-12">
            ...
            <p className="text-base md:text-lg xl:text-2xl text-[#4F200D]">คัดสรรแลนด์มาร์กสุดฮิต ถ่ายรูปสวย ทันกระแสก่อนใคร</p>
          </div>
              ...
              <Card key={dest.id} className="overflow-hidden border-0 shadow-xl rounded-2xl md:rounded-3xl bg-[#FFFDFA] group">
                <div className="relative h-42 md:h-54 lg:h-72 overflow-hidden">
                  <img src={dest.image} alt={dest.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
                    onError={(e) => { e.currentTarget.src = "https://images.unsplash.com/photo-1552465011-b4e21bf6e79a?w=600&q=80"; }} />
                </div>
                <CardContent className="p-3 md:p-4">
                  <div className="border-b border-[#E3DCD4] pb-2 md:pb-3 mb-2 md:mb-3">
                    <h3 className="text-lg md:text-2xl xl:text-[36px] font-bold text-[#4F200D] mb-1 md:mb-2">{dest.name}</h3>
                    <p className="text-xs md:text-sm xl:text-lg text-[#4F200D]/80 font-extralight">{dest.description}</p>
                  </div>
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs md:text-xs xl:text-base text-[#4F200D]/80 font-extralight">{dest.duration}</p>
                      <p className="text-base md:text-lg xl:text-2xl font-bold text-[#FF8400]">{dest.price}</p>
                    </div>
        ...
        <div className="max-w-[1920px] mx-auto px-4 md:px-8">
          ...
          <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-5 md:gap-6">
            {testimonials.map((testimonial) => (
              <Card key={testimonial.id} className="border-0 shadow-lg rounded-xl md:rounded-2xl bg-[#FFFDFA]">
                <CardContent className="p-4 md:p-6">
                  <div className="flex items-center gap-3 mb-3 md:mb-4">
                    <div className="w-12 h-12 md:w-16 md:h-16 rounded-full bg-[#4F200D]/10 flex items-center justify-center flex-shrink-0">
                      <span className="text-lg md:text-2xl font-bold text-[#4F200D]">{testimonial.name.charAt(0)}</span>
                    </div>
                    <div>
                      <p className="font-medium text-sm md:text-lg xl:text-xl text-[#4F200D]">{testimonial.name}</p>
                      <div className="flex gap-0.5 md:gap-1 mt-1">
                        {[...Array(5)].map((_, i) => <StarIcon key={i} size={12} />)}
                      </div>
                    </div>
                  </div>
                  <p className="text-xs md:text-sm xl:text-lg text-[#4F200D]/90 font-light leading-relaxed">"{testimonial.content}"</p>
                </CardContent>
              </Card>
            ))}
          </div>
      ...
      <section className="py-9 md:py-12 lg:py-18 bg-[#F6F1E9]">
        <div className="max-w-[1920px] mx-auto px-4 md:px-8">
          <div className="text-center">
            <h2 className="text-2xl md:text-4xl lg:text-5xl xl:text-[64px] font-bold text-[#4F200D] mb-3 md:mb-6">พร้อมเริ่มต้นการเดินทางหรือยัง?</h2>
            ...
            <div className="flex flex-col sm:flex-row gap-3 max-w-md md:max-w-lg mx-auto">
              <Input type="email" placeholder="กรอกอีเมลของคุณ" value={email} onChange={(e) => setEmail(e.target.value)}
