...
const getInputStyle = (hasError: boolean, extraBorder?: string): React.CSSProperties => ({
  width: '100%',
...
const Field: React.FC<FieldProps> = ({
  ...
  required = false, value, onChange, error, rightSlot,
}) => (
  <div style={{ marginBottom: 14 }}>
    <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#4E342E', marginBottom: 6 }}>
      {label}{required && <span style={{ color: '#FF8C00', marginLeft: 2 }}>*</span>}
    </label>
    <div style={{ position: 'relative' }}>
      <span style={{ position: 'absolute', left: 13, top: '50%', transform: 'translateY(-50%)', fontSize: 15, pointerEvents: 'none', opacity: 0.45 }}>
        {icon}
      </span>
      <input
        type={type}
...
const PasswordField: React.FC<PasswordFieldProps> = ({
  label, name, placeholder, icon, value, onChange, error, show, onToggle, borderColor, successMsg,
}) => (
  <div style={{ marginBottom: 14 }}>
    <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#4E342E', marginBottom: 6 }}>
    ...
    <div style={{ position: 'relative' }}>
      ...
      <input
        ...
        onBlur={e => {
          e.target.style.borderColor = error ? '#ef4444' : borderColor ?? '#E0D5CF';
          e.target.style.boxShadow = 'none';
          e.target.style.background = error ? '#fff5f5' : '#FAFAFA';
        }}
      />
      <button
        type="button"
        onClick={onToggle}
        style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', opacity: 0.4, fontSize: 16, lineHeight: 1, padding: 0, transition: 'opacity 0.2s' }}
    ...
    {error && <p style={{ color: '#ef4444', fontSize: 12, margin: '4px 0 0', fontWeight: 500 }}>{error}</p>}
    {successMsg && !error && <p style={{ color: '#22c55e', fontSize: 12, margin: '4px 0 0', fontWeight: 600 }}>{successMsg}</p>}
  </div>
);
...
const RegisterPage: React.FC = () => {
  ...
  const validateStep2 = (): boolean => {
    const e: Partial<FormData> = {};
    if (formData.password.length < 8) e.password = 'รหัสผ่านต้องมีอย่างน้อย 8 ตัว';
    if (formData.password !== formData.confirmPassword) e.confirmPassword = 'รหัสผ่านไม่ตรงกัน';
    setErrors(e);
    return Object.keys(e).length === 0;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setBanner({ type: 'success', msg: 'สมัครสมาชิกสำเร็จ! 🎉 กำลังพาไปหน้าเข้าสู่ระบบ...' });
      ...
      if (error.response?.status === 409) {
        ...
        setBanner({ type: 'error', msg: 'เกิดข้อผิดพลาด กรุณาลองใหม่อีกครั้ง' });
      }
    } finally {
      setLoading(false);
    }
          ...
          <div className="reg-card">
            ...
            <div className="reg-header">
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, position: 'relative', zIndex: 1 }}>
                <div style={{ width: 44, height: 44, background: 'rgba(255,255,255,0.22)', borderRadius: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22 }}>
                  🧭
                </div>
                <div>
                  <p style={{ color: 'rgba(255,255,255,0.72)', fontSize: 12, fontWeight: 500, margin: 0 }}>เริ่มต้นการเดินทาง</p>
                  <h2 style={{ color: 'white', fontSize: 22, fontWeight: 800, margin: 0, letterSpacing: '-0.4px' }}>สร้างบัญชีใหม่</h2>
                </div>
              </div>
            </div>
            ...
            <div className="reg-body">
              ...
              <div className="step-track">
                <div className={`step-seg ${step >= 1 ? 'step-done' : 'step-idle'}`} />
                <div className={`step-seg ${step === 2 ? 'step-active' : 'step-idle'}`} />
              </div>
              <p style={{ textAlign: 'center', fontSize: 11, color: '#BCAAA4', marginBottom: 18, fontWeight: 500 }}>
                {step === 1 ? 'ขั้นตอนที่ 1 จาก 2 — ข้อมูลส่วนตัว' : 'ขั้นตอนที่ 2 จาก 2 — ตั้งรหัสผ่าน'}
              </p>

              {/* Banner */}
              {banner && (
                <div className={`banner ${banner.type === 'success' ? 'banner-success' : 'banner-error'} slide-in`}>
              ...
              {step === 1 && (
                <div className="slide-in">
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 12px' }}>
                    <Field label="ชื่อ" name="firstName" placeholder="ชาญชัย" icon="👤" required value={formData.firstName} onChange={handleChange} error={errors.firstName} />
                    <Field label="นามสกุล" name="lastName" placeholder="ใจดี" icon="👤" required value={formData.lastName} onChange={handleChange} error={errors.lastName} />
                  </div>
                  ...
                  <p style={{ textAlign: 'center', fontSize: 13, marginTop: 16, color: '#8D6E63' }}>
                    มีบัญชีอยู่แล้ว?
                    <Link to="/login" style={{ color: '#FF8C00', fontWeight: 700, marginLeft: 5, textDecoration: 'none' }}>เข้าสู่ระบบ</Link>
                  </p>
                </div>
              )}
              ...
              {step === 2 && (
                <form onSubmit={handleSubmit} noValidate className="slide-in">
                  ...
                  <PasswordField
                    label="รหัสผ่าน" name="password" placeholder="อย่างน้อย 8 ตัวอักษร" icon="🔒"
                  ...
                  <PasswordField
                    label="ยืนยันรหัสผ่าน" name="confirmPassword" placeholder="กรอกรหัสผ่านอีกครั้ง" icon="🔑"
                    ...
                    successMsg={!errors.confirmPassword && formData.confirmPassword && formData.confirmPassword === formData.password ? '✓ รหัสผ่านตรงกัน' : undefined}
                  />

                  {/* Summary */}
                  <div className="summary-box">
                    <p style={{ fontSize: 12, fontWeight: 700, color: '#FF8C00', marginBottom: 10 }}>📋 ข้อมูลที่จะลงทะเบียน</p>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 16px' }}>
                      {([
                        ...
                        <div key={k}>
                          <p style={{ fontSize: 11, color: '#A1887F', margin: 0 }}>{k}</p>
                          <p style={{ fontSize: 12, fontWeight: 600, color: '#4E342E', margin: 0, wordBreak: 'break-all' }}>{v}</p>
                        </div>
                      ))}
                    </div>
                  </div>
