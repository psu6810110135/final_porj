...
- upload slip ปัจจุบันอัปเดตเฉพาะ `booking.paymentSlipUrl` + `booking.status`
- ยังไม่สร้าง/อัปเดต record ในตาราง `payments` ระหว่างอัปโหลด

3. **Payment verification completeness gap (High):**

- verify API ยังรับแค่ `status` (ไม่มี payload สำหรับเหตุผล)

4. **User management gap (High):**

- `UsersService.create()` และ `UsersService.remove()` ยังเป็น stub

5. **Config security gap (High):**

- JWT secret ยัง hardcode (`YOUR_SECRET_KEY`) ใน `auth.module.ts` และ `jwt.strategy.ts`
